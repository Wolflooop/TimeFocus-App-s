...
const CheckIcon = ({ done, size }) => (
  <Svg width={size || icon.sm} height={size || icon.sm} viewBox="0 0 24 24" fill="none">
    {done
      ? <Path d="M4 12l5 5L20 7" stroke="#fff" strokeWidth="2.5"
...
export default function TasksScreen({ onNavigate, onSubScreen, navHeight = 0, onTasksChanged }) {
  ...
  // Modal de acción (long press)
  const [actionTask,  setActionTask]  = useState(null); // tarea seleccionada
...
// FIX: carga inicial de tareas (faltaba este useEffect)
  useEffect(() => { loadTasks(); }, []);

  useEffect(() => {
    onSubScreen?.(showNew || (showEdit && !!editTask));
  }, [showNew, showEdit, editTask]);
  ...
  const toggleTask = async (task) => {
    ...
    // FIX: normalizar fecha_limite a YYYY-MM-DD — el API devuelve ISO completo
    // (ej. "2026-04-17T06:00:00.000Z") pero el backend solo acepta "2026-04-17"
    const payload = {
      ...task,
      estado: next,
    ...
    try {
      ...
      onTasksChanged?.(); // avisa al Dashboard que recargue
    } catch (e) { console.log(e); }
  };
  ...
  const deleteTask = async (task) => {
    try {
      ...
      onTasksChanged?.(); // avisa al Dashboard
    } catch (e) { console.log(e); }
    setShowAction(false);
  };

  const openAction = (task) => {
    setActionTask(task);
    setShowAction(true);
  };
  ...
  // Filtrar: excluir completadas (se cuentan aparte)
  const allFiltered = tasks.filter(t => {
    const today = new Date().toISOString().slice(0, 10);
    // Normaliza la fecha (puede venir como ISO completo del API)
    const fechaNorm = t.fecha_limite ? String(t.fecha_limite).slice(0, 10) : '';
    if (filter === 'Hoy')        return fechaNorm === today;
    if (filter === 'Semana') {
      const diff = (new Date(fechaNorm) - new Date()) / 86400000;
  ...
  const normal    = pending.filter(t => t.prioridad === 'baja' || !['alta','media'].includes(t.prioridad));

  if (showNew) {
    return <NewTaskScreen onBack={() => { setShowNew(false); loadTasks(); onTasksChanged?.(); }} />;
  }

  if (showEdit && editTask) {
    return (
      <EditTaskScreen
        task={editTask}
        onBack={() => { setShowEdit(false); loadTasks(); onTasksChanged?.(); }}
      />
    );
  }
    ...
    return (
      <KanbanScreen
        tasks={tasks.map(t => ({
          ...
          fecha: t.fechaVencimiento ? new Date(t.fechaVencimiento).toLocaleDateString('es-MX',{day:'numeric',month:'short'}) : '',
          prioridad: t.prioridad || 'baja',
          status: t.estado === 'completada' ? 'done' : t.estado === 'en_progreso' ? 'in_progress' : 'pending',
          progress: t.progreso || 0,
        }))}
        onBack={() => setShowKanban(false)}
        onAddTask={() => { setShowKanban(false); setShowNew(true); }}
        onMoveTask={(taskId, newStatus) => {
          // Update locally; a full implementation would call api.patch
          setTasks(prev => prev.map(t =>
            String(t._id || t.id) === taskId
              ? { ...t, estado: newStatus === 'done' ? 'completada' : newStatus === 'in_progress' ? 'en_progreso' : 'pendiente' }
              : t
          ));
          onTasksChanged?.();
        }}
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity style={s.gridBtn} activeOpacity={0.7} onPress={() => setShowKanban(true)}>
          <GridIcon />
        </TouchableOpacity>
      </View>
      ...
      {completadasCount > 0 && (
        <View style={s.doneBar}>
          ...
          <Text style={s.doneBarText}>
            {completadasCount} tarea{completadasCount > 1 ? 's' : ''} completada{completadasCount > 1 ? 's' : ''} hoy ✓
          </Text>
        </View>
      )}
          ...
          {pending.length === 0 && (
            <View style={s.empty}>
              ...
              <Text style={s.emptyTitle}>
                {completadasCount > 0 ? '¡Todo listo!' : 'No hay tareas aquí'}
              </Text>
              <Text style={s.emptyText}>
                {completadasCount > 0
                  ? `Completaste ${completadasCount} tarea${completadasCount > 1 ? 's' : ''}`
      ...
      {/* FAB — bottom se suma al navbar para no quedar tapado */}
      <TouchableOpacity
            ...
            {actionTask && (
              <>
                ...
                <Text style={m.sheetSub}>
                  {actionTask.materia || 'Sin materia'} ·{' '}
                  {PRIO_CONFIG[actionTask.prioridad]?.label.replace('• ', '') || 'Normal'}
                </Text>
                ...
                <TouchableOpacity style={m.row} onPress={openEdit} activeOpacity={0.7}>
                  ...
                  <View style={m.rowInfo}>
                    <Text style={m.rowTitle}>Editar tarea</Text>
                    <Text style={m.rowDesc}>Modifica título, fecha, prioridad u otros campos</Text>
                  </View>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  style={m.row}
                  onPress={() => { toggleTask(actionTask); setShowAction(false); }}
                  activeOpacity={0.7}>
                  <View style={[m.rowIcon, { backgroundColor: '#E8F5E9' }]}>
                    <CheckIcon done size={icon.sm} />
                  </View>
                  <View style={m.rowInfo}>
                    <Text style={m.rowTitle}>
                      {actionTask.estado === 'completada' ? 'Marcar como pendiente' : 'Marcar como completada'}
                    </Text>
                    <Text style={m.rowDesc}>
                      {actionTask.estado === 'completada'
                        ? 'Mover de regreso a pendientes'
                        : 'Registrarla en el contador de completadas'}
                    </Text>
                  </View>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  style={m.row}
                  onPress={() => deleteTask(actionTask)}
                  activeOpacity={0.7}>
                  <View style={[m.rowIcon, { backgroundColor: '#FFEBEE' }]}><TrashIcon /></View>
                  <View style={m.rowInfo}>
                    <Text style={[m.rowTitle, { color: '#FF5252' }]}>Eliminar tarea</Text>
                    <Text style={m.rowDesc}>Se borra permanentemente</Text>
                  </View>
                </TouchableOpacity>
...
function TaskItem({ task, onToggle, onLongPress }) {
  ...
  // Barra de progreso según prioridad
  const progreso = task.prioridad === 'alta' ? 0.85
    : task.prioridad === 'media'             ? 0.45
    : 0.15;
    ...
    <TouchableOpacity
      ...
      <View style={s.taskBody}>
        ...
        <Text style={s.taskMeta}>
          ...
          {task.fecha_limite ? `Vence ${String(task.fecha_limite).slice(0, 10)}` : 'Sin fecha'}
        </Text>
        <View style={s.progressBg}>
          <View style={[s.progressFg, { width: `${progreso * 100}%`, backgroundColor: prio.text }]} />
        </View>
      </View>

      {/* Badge de prioridad */}
      <View style={[s.badge, { backgroundColor: prio.bg }]}>
        <Text style={[s.badgeText, { color: prio.text }]}>{prio.label}</Text>
      </View>
    </TouchableOpacity>
  );
}
...
// ── EditTaskScreen ─────────────────────────────────────────────────
function EditTaskScreen({ task, onBack }) {
  ...
  const [date,       setDate]       = useState(
    task.fecha_limite ? new Date(task.fecha_limite) : new Date()
  );
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await api.put(`/tasks/${task.id_tarea}`, {
        titulo:       form.titulo,
      ...
      setErrors({ general: 'Error al guardar la tarea' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={ns.root}>
      <View style={ns.header}>
        <TouchableOpacity onPress={onBack} style={ns.backBtn}>
          <BackIcon />
        </TouchableOpacity>
        <Text style={ns.title}>Editar tarea</Text>
      ...
      <ScrollView contentContainerStyle={ns.scroll} keyboardShouldPersistTaps="handled">
        ...
        <TextInput
          style={[ns.inputBox, errors.titulo && ns.inputError]}
          placeholder="Ej: Entregar reporte de BD"
          placeholderTextColor="#aaa"
          value={form.titulo}
          onChangeText={t => { setForm(f => ({ ...f, titulo: t })); setErrors(e => ({ ...e, titulo: null })); }}
        />
        {errors.titulo && <Text style={ns.error}>{errors.titulo}</Text>}
        ...
        <View style={ns.row}>
          {PRIORIDADES.map(p => (
            <TouchableOpacity
              ...
              style={[
                ns.pill,
                form.prioridad === p.toLowerCase() && {
                  borderColor: prioColor[p],
                  backgroundColor: `${prioColor[p]}18`,
                },
              ]}
              onPress={() => setForm(f => ({ ...f, prioridad: p.toLowerCase() }))}>
              <Text style={[
                ns.pillText,
                form.prioridad === p.toLowerCase() && { color: prioColor[p], fontWeight: '700' },
              ]}>
                {p}
              </Text>
            </TouchableOpacity>
        ...
        {errors.general && <Text style={ns.error}>{errors.general}</Text>}

        <TouchableOpacity
          style={[ns.btnCreate, loading && { opacity: 0.7 }]}
          onPress={handleSave}
          disabled={loading}
          activeOpacity={0.85}>
          {loading
            ? <ActivityIndicator color={colors.primary} />
            : <Text style={ns.btnCreateText}>Guardar cambios ✓</Text>
          }
        </TouchableOpacity>
      </ScrollView>
    </View>
...
// ── NewTaskScreen ──────────────────────────────────────────────────
function NewTaskScreen({ onBack }) {
  const [form, setForm] = useState({
    titulo: '', materia: '', fecha_limite: '',
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setErrors({ general: 'Error al crear la tarea' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={ns.root}>
      ...
      <ScrollView contentContainerStyle={ns.scroll} keyboardShouldPersistTaps="handled">
        ...
        <TextInput
          style={[ns.inputBox, { height: rv(80, 60, 100), textAlignVertical: 'top' }]}
          placeholder="Ej: Revisar capítulos 3 y 4 antes de la entrega..."
          placeholderTextColor="#aaa"
          multiline
          value={form.descripcion}
          onChangeText={t => setForm(f => ({ ...f, descripcion: t }))}
        ...
        <TouchableOpacity
          ...
          {loading
            ? <ActivityIndicator color={colors.primary} />
            : <Text style={ns.btnCreateText}>Crear tarea →</Text>
          }
        </TouchableOpacity>
      </ScrollView>
    </View>
...
const s = StyleSheet.create({
  ...
  header:         { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
                    paddingHorizontal: space.screen, paddingTop: rv(16,12,20), paddingBottom: rv(8,6,12) },
  title:          { fontSize: clamp(font.xl,18,26), fontWeight: '800', color: colors.primary },
  gridBtn:        { width: clamp(W*0.09,36,48), height: clamp(W*0.09,36,48),
  ...
  chip:           { paddingHorizontal: rs(14,10,18), paddingVertical: rv(6,4,8),
  ...
  doneBar:        { flexDirection: 'row', alignItems: 'center', gap: space.sm,
                    marginHorizontal: space.screen, marginBottom: rv(8,5,12),
                    backgroundColor: '#E8F5E9', borderRadius: radius.sm,
                    paddingHorizontal: rs(12,9,16), paddingVertical: rv(8,6,10) },
  ...
  taskCard:       { backgroundColor: '#fff', borderRadius: radius.md,
                    padding: rs(14,10,18), marginBottom: rv(8,6,10),
                    flexDirection: 'row', alignItems: 'center',
                    gap: rs(12,8,16), borderWidth: 0.5, borderColor: '#E8ECF4' },
  cb:             { width: clamp(W*0.055,20,28), height: clamp(W*0.055,20,28),
                    borderRadius: radius.sm, borderWidth: 1.5, borderColor: '#D0D8E8',
  ...
  badge:          { paddingHorizontal: rs(8,6,10), paddingVertical: 3,
                    borderRadius: radius.full, alignSelf: 'center', flexShrink: 0 },
  ...
  fab:            {
    position: 'absolute', bottom: rv(28, 20, 36), right: rs(20,14,28),
...
const m = StyleSheet.create({
  ...
  sheet:        { backgroundColor: '#fff', borderTopLeftRadius: radius.xl,
  ...
  handle:       { width: 36, height: 4, borderRadius: 2, backgroundColor: '#E0E0E0',
                  alignSelf: 'center', marginBottom: rv(16,10,22) },
  ...
  row:          { flexDirection: 'row', alignItems: 'center', gap: rs(14,10,18),
                  paddingVertical: rv(14,10,18),
                  borderBottomWidth: 0.5, borderBottomColor: '#F0F0F5' },
  rowIcon:      { width: clamp(W*0.11,40,52), height: clamp(W*0.11,40,52),
                  borderRadius: radius.md, backgroundColor: '#F5F5F8',
                  alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  rowInfo:      { flex: 1 },
...
const ns = StyleSheet.create({
  ...
  inputBox:         { borderWidth: 1, borderColor: '#E0E0E0', borderRadius: radius.sm,
                      padding: rs(13,10,16), backgroundColor: '#FAFAFA',
                      minHeight: clamp(H*0.055,44,56), justifyContent: 'center' },
  inputError:       { borderColor: '#FF5252' },
  row:              { flexDirection: 'row', gap: space.sm },
  pill:             { flex: 1, paddingVertical: rv(10,7,13), borderRadius: radius.sm,
                      borderWidth: 1.5, borderColor: '#E0E0E0',
                      alignItems: 'center', backgroundColor: '#FAFAFA' },
  pillText:         { fontSize: font.sm, color: '#888', fontWeight: '500' },
  pillActiveYellow: { borderColor: '#F5BF23', backgroundColor: '#FFF8E1' },
  btnCreate:        { backgroundColor: colors.accent, paddingVertical: rv(16,12,20),
                      borderRadius: radius.md, alignItems: 'center',
                      marginTop: rv(24,16,32), marginBottom: rv(16,10,20) },
